...
params.net = loadNet_qx3channel_diffSigma111([rows,cols,3], use_gpu);%%%%%%%%%%   model
% run EDAEPRec
params.sigma_net = 9;   %%%%%%%%%%%%%%%%%%%%%%%   noise_level

[reconstruction,pwls,psnr_psnr] = CTRec(original_image, reconstruction, SystemMatrix, maxvalue_exchange,pre_pwls,params);
toc
fclose('all');
...
save(strcat('./HF-DAEP_result_N9/HF_DAEP_',imageName,'_',num2str(pro_views),'_image'),'pwls');
...
save(strcat('./HF-DAEP_result_N9/HF_DAEP_',imageName,'_',num2str(pro_views),'_psnr'),'psnr_psnr')
